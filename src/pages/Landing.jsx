...
export default function Landing() {
  ...
  return (
    <div dir="rtl" className="min-h-screen bg-[#F8FAFC]" style={{ fontFamily: "'Rubik', sans-serif" }}>
      {/* Navigation */}
      ...
      <section className="py-16 px-4 bg-[#F2E9DB]/30">
        <div className="max-w-3xl mx-auto">
          <h2 className="text-2xl md:text-3xl font-bold text-[#0B1220] text-center mb-8">
            שאלות נפוצות
          </h2>
          <div className="space-y-2">
            {t.faq.map((item, i) => (
              <Card 
                key={i}
                className="cursor-pointer border-0 shadow-sm hover:shadow-md transition-shadow rounded-lg overflow-hidden"
                onClick={() => setOpenFaq(openFaq === i ? null : i)}
              >
                <CardContent className="p-0">
                  <div className="flex items-center justify-between p-3 bg-white">
                    <span className="font-medium text-[#0B1220] text-sm">{item.q}</span>
                    {openFaq === i ? (
                      <ChevronUp className="h-4 w-4 text-gray-400 flex-shrink-0 mr-2" />
                    ) : (
                      <ChevronDown className="h-4 w-4 text-gray-400 flex-shrink-0 mr-2" />
                    )}
                  </div>
                  {openFaq === i && (
                    <div className="px-3 pb-3 bg-white text-gray-600 text-sm">
                      {item.a}
                    </div>
                  )}
                </CardContent>
      ...
      <section className="py-20 px-4 bg-gradient-to-br from-[#0B1220] to-[#1a2744]">
        <div className="max-w-3xl mx-auto text-center">
          ...
          >
            ...
            <p className="text-lg text-white/70 mb-8">פלטפורמה מקצועית לניהול מלא של נכסי האירוח שלכם</p>
            <Button 
              size="lg" 
